...
Validation involves multiple stages (consistency checking, counterfactual analysis, minimality scoring), each producing structured results. Rather than scattering data definitions across modules, this package centralizes them, ensuring a single source of truth for all input/output data shapes. This prevents duplication and guarantees that all consumers agree on the same field names, types, and semantics.
...
├── semantic_result.py             # SemanticValidationResult, ConsistencyIssue, ContradictionIssue, OverfittingIndicator
...
- **`SimulationTrace`** (frozen dataclass) — A single simulation run containing:
...
- **`SimulationDataset`** — A collection of traces with utility methods:
  - `filter_by_outcome(outcome)` — Filter traces by Pass/Fail
  - `get_all_variables()` — Extract all variable names used across traces
  - `split_train_test(test_ratio)` — Split into training/test sets for overfitting detection
...
- **`CounterfactualPair`** (frozen dataclass) — Links an original input `x` to a counterfactual `x'`:
...
- **`CounterfactualEvidence`** — Collection of pairs for a specific rule:
  - `inconsistent_rule`: The rule being analyzed
...
|-------|---------|------------|
| `ConsistencyIssue` | A single mismatch between rule verdict and observed outcome | `trace`, `rule_verdict`, `observed_outcome` |
| `ContradictionIssue` | A contradiction between current and historical rules | `current_rule`, `historical_rule`, `conflicting_input`, `explanation` |
| `OverfittingIndicator` | Evidence of overfitting | `indicator_type`, `severity`, `affected_variables`, `evidence` |
| `SemanticValidationResult` | Complete semantic validation output | `consistency_score`, `contradictions`, `overfitting_risk`, `passed_validation` |

**Indicator types:** `"boundary_sensitive"`, `"overly_specific"`, `"train_test_gap"`
...
- **`RelationChange`** — A single change in one relation between original and refined rules:
  - `variable`, `operator`, `original_constant`, `refined_constant`
  - `delta`, `magnitude`, `change_type` (`"tightening"`, `"loosening"`, `"unchanged"`)
  - `is_justified`: Whether counterfactual evidence supports this change
  - Rich `__str__()` with direction arrows and justification marks
...
|----------|-------------|
| `semantic/consistency_checker.py` | `SimulationTrace`, `SimulationDataset`, `ConsistencyIssue` |
| `semantic/contradiction_checker.py` | `ContradictionIssue` |
| `semantic/overfitting_detector.py` | `OverfittingIndicator`, `SimulationDataset` |
| `semantic/counterfactual_generator.py` | `CounterfactualPair`, `CounterfactualEvidence` |
| `semantic/semantic_validator.py` | `SemanticValidationResult` |
| `minimality/change_extractor.py` | `RelationChange` |
| `minimality/minimality_validator.py` | `MinimalityResult`, `CounterfactualEvidence` |
| `examples/` | All data classes for demo construction |
| `cbf_data/adapter.py` | `SimulationTrace`, `SimulationDataset` |
